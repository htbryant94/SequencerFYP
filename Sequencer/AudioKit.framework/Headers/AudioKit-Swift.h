...
/**
  ...
  <ul>
    <li>
      \code
      floatChannelData?[0]
      \endcode will contain an Array of left channel samples as \code
      Float
      \endcode
    </li>
    <li>
      \code
      floatChannelData?[1]
      \endcode will contains an Array of right channel samples as \code
      Float
      \endcode
    </li>
  </ul>
...
/**
  ...
  \param scheduledTime Time in the future to start playback. This is useful for scheduling a group of sounds to
  start concurrently, or to simply schedule the start time.

*/
- (void)playFrom:(double)time to:(double)endTime when:(double)scheduledTime;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
- (nonnull instancetype)initWithAvAudioNode:(AVAudioNode * _Nonnull)avAudioNode attach:(BOOL)attach SWIFT_UNAVAILABLE;
@end
...
*/
- (nonnull instancetype)init:(AKNode * _Nonnull)input x:(double)x y:(double)y OBJC_DESIGNATED_INITIALIZER;
/**
  Function to start, play, or activate the node, all do the same thing
*/
- (void)start;
...
*/
- (nullable instancetype)initWithNode:(AKNode * _Nonnull)node file:(AKAudioFile * _Nullable)file error:(NSError * _Nullable * _Nullable)error OBJC_DESIGNATED_INITIALIZER;
/**
  Start recording
*/
...
*/
- (nonnull instancetype)init:(AKNode * _Nonnull)input amount:(double)amount OBJC_DESIGNATED_INITIALIZER;
/**
  Function to start, play, or activate the node, all do the same thing
*/
- (void)start;
...
- (AUParameter * _Nullable)objectForKeyedSubscript:(NSString * _Nonnull)key;
@end
...
/**
  \code
  UIGestureRecognizer
  \endcode subclass which tracks the state of individual touches.
*/
SWIFT_CLASS("_TtC8AudioKit27MultitouchGestureRecognizer")
@interface MultitouchGestureRecognizer : UIGestureRecognizer
/**
  The maximum number of touches allowed in the stack/queue. Defaults to \code
  0
  \endcode, signifying unlimited touches.
  If \code
  count
  \endcode is decreased past the current number of touches, any excess touches will be ended immediately.
*/
...
/**
  ...
  true
  \endcode, when touches end they will be retained in \code
  touches
  \endcode until such time as all touches have ended and a new touch begins.
  If \code
  sustain
  \endcode is switched from \code
  true
  \endcode to \code
  false
  \endcode, any currently sustained touches will be ended immediately.
*/
@property (nonatomic) BOOL sustain;
/**
  The currently tracked collection of touches. May contain touches after they have ended, if \code
  sustain
  \endcode is set to \code
  true
  \endcode.
...
/**
  ...
  \endcode which allows the delegate to receive messages relating to individual touches. The \code
  delegate
  \endcode property can be set to a class implementing \code
  MultitouchGestureRecognizerDelegate
  \endcode and it will receive these messages.
*/
